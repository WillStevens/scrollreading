...
#include "../zarrlike/zarr_c32i1b1024.c"
...
#define VECTORFIELD_CONSTANT (0.05f/73.0f) // factor of 1/73.0f was added when changing to single-byte vector field 
...
float GetDistanceAtPoint(const pointInt &p)
{
	...
	{
		distanceLookup[p] = ZARRRead_c32i1b1024(vectorField,zp,yp,xp,3);
	}
...
void SetVectorField(int x, int y)
{
  ...
  if (px-VOL_OFFSET_X>=0 && px-VOL_OFFSET_X<VF_SIZE && py-VOL_OFFSET_Y>=0 && py-VOL_OFFSET_Y<VF_SIZE && pz-VOL_OFFSET_Z>=0 && pz-VOL_OFFSET_Z<VF_SIZE)
  {
	...
	{
		vectorFieldLookup[p] = point(ZARRRead_c32i1b1024(vectorField,pz,py,px,0)*VECTORFIELD_CONSTANT,ZARRRead_c32i1b1024(vectorField,pz,py,px,1)*VECTORFIELD_CONSTANT,ZARRRead_c32i1b1024(vectorField,pz,py,px,2)*VECTORFIELD_CONSTANT);
	}
  ...
  for(int y = 0; y<SHEET_SIZE; y++)
	if (active[x][y])
	{
	  ...
	  if (SETot/SENum > HIGH_STRESS_THRESHHOLD)
	  {
		...
		int xb = (int)((paperPos[x][y][0]-VOL_OFFSET_X)/STRESS_BLOCK_SIZE);
		int yb = (int)((paperPos[x][y][1]-VOL_OFFSET_Y)/STRESS_BLOCK_SIZE);
		int zb = (int)((paperPos[x][y][2]-VOL_OFFSET_Z)/STRESS_BLOCK_SIZE);
...
{ 
  ...
  if (argc != 9+4)
  {
	printf("Usage: simpaper6 <seed> <vector field zarr> <surface points output> <stress output>\n");
  ...
  vectorField = ZARROpen_c32i1b1024(argv[9+1]);
  ...
  for(int i = 0; i<300; i++)
  {
    printf("--- %d ---\n",i);
    int j = 0;
    while (j<10 || (Forces()>RELAX_FORCE_THRESHHOLD && j<MAX_RELAX_ITERATIONS))
	{
      j++;

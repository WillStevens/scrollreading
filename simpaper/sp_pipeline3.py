...
def StartPatchCoordFile(patch,iterations,coords):
  f = open("d:/pipelineOutput/patchCoords.txt","w")
  f.write("ABS %d %d %f %f %f\n" % (patch,iterations,coords[0],coords[1],coords[2]))
  f.close()
  
def AddToPatchCoordFile(patch,iterations,l):
  f = open("d:/pipelineOutput/patchCoords.txt","a")
  for i in l:
    f.write("REL " + str(patch) + " " + str(iterations))
    for n in i:
      f.write(" "+str(n))
    f.write("\n")
  f.close()
...
while True:
  ...
  if not restart:
    ...
    Step("simpaper7")
    iterations = Call(["./simpaper7"] + [str(x) for x in seed] + ["d:/pvfs_2048_chunk_32_v2.zarr",patch,outputDir+"/stress_"+str(patchNum)+".csv"])
  ...
  if patchNum==0:
    ...
    StartPatchCoordFile(patchNum,iterations,(0,0,0))
    ...
    if (aligned):
      ...
      if not VarianceTest(variance):
        ...
        if VarianceTest(variance):
          AddToPatchCoordFile(patchNum,iterations,CallAlignMulti(currentSurface,patchif))
        ...
        AddToPatchCoordFile(patchNum,iterations,CallAlignMulti(currentSurface,patchi))
        Step("transform_patch")

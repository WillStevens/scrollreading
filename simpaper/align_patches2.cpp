...
typedef std::tuple<int,int,float,float,float> gridPoint;

#define SHEET_SIZE_X 3000
#define SHEET_SIZE_Y 6000

float paperPos[2][SHEET_SIZE_X][SHEET_SIZE_Y][3];
bool active[2][SHEET_SIZE_X][SHEET_SIZE_Y];
...
void LoadPointSet(const char *fname, int index)
{
	...
	if (f)
	{
	    while (fscanf(f,"%f,%f,%f,%f,%f\n",&x,&y,&xp,&yp,&zp)==5)
	    {
		  ...
		  paperPos[index][(int)x][(int)y][0] = xp;
		  paperPos[index][(int)x][(int)y][1] = yp;
		  paperPos[index][(int)x][(int)y][2] = zp;
		  
		  active[index][(int)x][(int)y] = true;
	    }
...
// use paperPos[1] to fill cell map
void FillCellMap(void)
{
	for(int x0 = 0; x0<SHEET_SIZE_X; x0++)
	for(int y0 = 0; y0<SHEET_SIZE_Y; y0++)
		if (active[0][x0][y0])
	    {
		  int xc0 = ((int)paperPos[0][x0][y0][0])/CELL_SIZE;
		  int yc0 = ((int)paperPos[0][x0][y0][1])/CELL_SIZE;
		  int zc0 = ((int)paperPos[0][x0][y0][2])/CELL_SIZE;

          gridCell g(xc0,yc0,zc0);

          if (cellMap0.count(g)==0)
              cellMap0[g] = std::vector<gridPoint>();			  
		  cellMap0[g].push_back( gridPoint(x0,y0,paperPos[0][x0][y0][0],paperPos[0][x0][y0][1],paperPos[0][x0][y0][2]) );
	    }

	for(int x1 = 0; x1<SHEET_SIZE_X; x1++)
	for(int y1 = 0; y1<SHEET_SIZE_Y; y1++)
		if (active[1][x1][y1])
	    {
		  int xc1 = ((int)paperPos[1][x1][y1][0])/CELL_SIZE;
		  int yc1 = ((int)paperPos[1][x1][y1][1])/CELL_SIZE;
		  int zc1 = ((int)paperPos[1][x1][y1][2])/CELL_SIZE;

          gridCell g(xc1,yc1,zc1);

          if (cellMap1.count(g)==0)
              cellMap1[g] = std::vector<gridPoint>();			  
		  cellMap1[g].push_back( gridPoint(x1,y1,paperPos[1][x1][y1][0],paperPos[1][x1][y1][1],paperPos[1][x1][y1][2]) );
	    }
}
...
int main(int argc, char * argv[])
{
	if (argc != 3)
	{
		printf("Usage: align_patches <patch 1> <patch 2>\n");
		exit(-1);
	}

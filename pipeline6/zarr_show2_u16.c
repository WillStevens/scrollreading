	...
	if (1==1)
	{
		...
		if (tif)
		{
			...
			for(row=0; row<height;row++)
			{
				...
				{
					unsigned pixel = ZARRRead_4(za,zcoord,ycoord+row,xcoord+i);
				   ((uint8_t *)buf)[3*i] = (pixel>>8)/2;
				   ((uint8_t *)buf)[3*i+1] = (pixel>>8)/2;
				   ((uint8_t *)buf)[3*i+2] = (pixel>>8)/2;

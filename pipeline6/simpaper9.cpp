...
bool InitialiseSeed(void)
{
  ...
  // Check that the seed is on a surface point
  ZARR_1 *z = ZARROpen_1(SURFACE_ZARR);
  
  if (!z)
	  return false;
  
  if (ZARRRead_1(z,seeds[currentSeed][2],seeds[currentSeed][1],seeds[currentSeed][0]) != 255)
	  return false;

...
{
	pointInt q;
	q[0] = ((int)p.x)/NEIGHBOUR_RADIUS;
	q[1] = ((int)p.y)/NEIGHBOUR_RADIUS;
	q[2] = ((int)p.z)/NEIGHBOUR_RADIUS;
...
float PatchGenerator::GetDistanceAtPoint(const pointInt &p)
{
	int xp = ((int)p[0]);
	int yp = ((int)p[1]);
	int zp = ((int)p[2]);
...
bool PatchGenerator::HasCloseNeighbour(const point &p)
{
	...
	pointInt q;
	for(q[0] = xp-1; q[0]<=xp+1; q[0]++)
	for(q[1] = yp-1; q[1]<=yp+1; q[1]++)
	for(q[2] = zp-1; q[2]<=zp+2; q[2]++)
	{
		for(const point &r : pointLookup[q])
		{
			if (PointDistance(p,r)<=NEIGHBOUR_RADIUS_FLOAT)
			{
				return true;
			}
		}
...
void PatchGenerator::SetVectorField(int x, int y)
{
  ...
  if (it == vectorFieldLookup.end())
  {
    ...
    {	
		v.x = ZARRRead_c64i1b256(vectorField,pz,py,px,0)*VECTORFIELD_CONSTANT;
		v.y = ZARRRead_c64i1b256(vectorField,pz,py,px,1)*VECTORFIELD_CONSTANT;
		v.z = ZARRRead_c64i1b256(vectorField,pz,py,px,2)*VECTORFIELD_CONSTANT;
	}
...
{  
  ...
  for(int ai = 0; ai<activeListSize; ai++)
  {
    ...
    float forceMag = DotProduct(acc,acc);
...
bool PatchGenerator::TryToFill(int xp, int yp, float &rp0, float &rp1, float &rp2)
{
  if (!active[xp][yp])
  {
    for(int i = 0; i<4; i++)
...
*/	  
	  ...
	  if (xo2>=0 && xo2<SHEET_SIZE && yo2>=0 && yo2<SHEET_SIZE && active[xo1][yo1] && active[xo2][yo2] && xc1>=0 && xc1<SHEET_SIZE && yc1>=0 && yc1<SHEET_SIZE && xd1>=0 && xd1<SHEET_SIZE && yd1>=0 && yd1<SHEET_SIZE && active[xc1][yc1] && active[xd1][yd1])
	  {
		...
		Vec3 rp = (paperSheet[xo1][yo1].pos - paperSheet[xo2][yo2].pos)*2;
      ...
      if (xc1>=0 && xc1<SHEET_SIZE && yc1>=0 && yc1<SHEET_SIZE && active[xo1][yo1] && active[xa1][ya1] && active[xc1][yc1])
	  {
		  ...
		  Vec3 rp = (paperSheet[xo1][yo1].pos+paperSheet[xa1][ya1].pos)/2;
...
void PatchGenerator::PatchGenerator(std::string surfaceZarrName);
{
	surfaceZarr = ZARROpen_1(surfaceZarrName.c_str());
    vectorFieldZarr = ZARROpen_c64i1b256(argv[9+1]);

    activeListSize = 0;	
}
		
void PatchGenerator::~PatchGenerator(void);
{
...
int PatchGenerator::GeneratePatch(void)
{ 

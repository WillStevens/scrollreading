...
#include <sys/types.h>
#include <blosc2.h>
typedef float ZARRType_6;

typedef struct {
    int locationRootLength;
    char *location;
  
    unsigned char compressedData[sizeof(ZARRType_6)*131072+BLOSC2_MAX_OVERHEAD];
    ZARRType_6 buffers[5000][32][32][32][4];
    int bufferIndex[5000][4];
    unsigned char written[5000];
    uint64_t bufferUsed[5000];
    ZARRType_6 (*buffer)[32][32][32][4];
...
ZARR_6 *ZARROpen_6(const char *location)
{
	ZARR_6 *z = (ZARR_6 *)malloc(sizeof(ZARR_6));
...
int ZARRFlushOne_6(ZARR_6 *z, int i)
{
    if (z->written[i])
	{
	  ...
	  int compressed_len = blosc2_compress(3,1,sizeof(ZARRType_6),z->buffers[i],sizeof(ZARRType_6)*131072,z->compressedData,sizeof(ZARRType_6)*131072+BLOSC2_MAX_OVERHEAD);
...
int ZARRCheckChunk_6(ZARR_6 *z, int c[4])
{	
	...
	for(z->index = 0; z->index < 5000; z->index++)
	{
	...
	if (!f)
	{
		...
		memset(z->buffer,0,sizeof(ZARRType_6)*131072);
	...
	if (f)
	{
        ...
        blosc1_set_compressor("zstd");
        int decompressed_size = blosc2_decompress(z->compressedData, fsize, z->buffer, sizeof(ZARRType_6)*131072);
...
ZARRType_6 ZARRRead_6(ZARR_6 *za,int x0,int x1,int x2,int x3)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
void ZARRReadN_6(ZARR_6 *za,int x0,int x1,int x2,int x3,int n,ZARRType_6 *v)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
int ZARRWrite_6(ZARR_6 *za,int x0,int x1,int x2,int x3,ZARRType_6 value)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
void ZARRWriteN_6(ZARR_6 *za,int x0,int x1,int x2,int x3,int n, ZARRType_6 *v)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
void ZARRNoCheckWriteN_6(ZARR_6 *za,int x0,int x1,int x2,int x3,int n, ZARRType_6 *v)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;

...
#include <sys/types.h>
#include <blosc2.h>
typedef int8_t ZARRType_c32i1b64;

typedef struct {
    int locationRootLength;
    char *location;
  
    unsigned char compressedData[sizeof(ZARRType_c32i1b64)*131072+BLOSC2_MAX_OVERHEAD];
    ZARRType_c32i1b64 buffers[64][32][32][32][4];
    int bufferIndex[64][4];
    unsigned char written[64];
    uint64_t bufferUsed[64];
    ZARRType_c32i1b64 (*buffer)[32][32][32][4];
...
ZARR_c32i1b64 *ZARROpen_c32i1b64(const char *location)
{
	ZARR_c32i1b64 *z = (ZARR_c32i1b64 *)malloc(sizeof(ZARR_c32i1b64));
...
int ZARRFlushOne_c32i1b64(ZARR_c32i1b64 *z, int i)
{
    if (z->written[i])
	{
	  ...
	  int compressed_len = blosc2_compress(3,1,sizeof(ZARRType_c32i1b64),z->buffers[i],sizeof(ZARRType_c32i1b64)*131072,z->compressedData,sizeof(ZARRType_c32i1b64)*131072+BLOSC2_MAX_OVERHEAD);
...
int ZARRFlush_c32i1b64(ZARR_c32i1b64 *z)
{
	for(int i = 0; i<64; i++)
	{
		if (z->bufferIndex[i][0] != -1)
		{
			ZARRFlushOne_c32i1b64(z,i);
		}
	}
...
int ZARRClose_c32i1b64(ZARR_c32i1b64 *z)
{
    ZARRFlush_c32i1b64(z);
    free(z->location);
    free(z);
...
int ZARRCheckChunk_c32i1b64(ZARR_c32i1b64 *z, int c[4])
{	
	...
	if (z->index == 64)
	{
		...
		ZARRFlushOne_c32i1b64(z,oldestIndex);
		z->index = oldestIndex;
	}
	...
	if (!f)
	{
		...
		memset(z->buffer,0,sizeof(ZARRType_c32i1b64)*131072);
	...
	if (f)
	{
        ...
        blosc1_set_compressor("zstd");
        int decompressed_size = blosc2_decompress(z->compressedData, fsize, z->buffer, sizeof(ZARRType_c32i1b64)*131072);
...
ZARRType_c32i1b64 ZARRRead_c32i1b64(ZARR_c32i1b64 *za,int x0,int x1,int x2,int x3)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
void ZARRReadN_c32i1b64(ZARR_c32i1b64 *za,int x0,int x1,int x2,int x3,int n,ZARRType_c32i1b64 *v)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
	...
	memcpy(v,&(*za->buffer)[m[0]][m[1]][m[2]][m[3]],n*sizeof(ZARRType_c32i1b64));
}

int ZARRWrite_c32i1b64(ZARR_c32i1b64 *za,int x0,int x1,int x2,int x3,ZARRType_c32i1b64 value)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
void ZARRWriteN_c32i1b64(ZARR_c32i1b64 *za,int x0,int x1,int x2,int x3,int n, ZARRType_c32i1b64 *v)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
	...
	memcpy(&(*za->buffer)[m[0]][m[1]][m[2]][m[3]],v,n*sizeof(ZARRType_c32i1b64));

	za->written[za->index] = 1;  
}

// Assumes that we have already written at least once to this chunk
void ZARRNoCheckWriteN_c32i1b64(ZARR_c32i1b64 *za,int x0,int x1,int x2,int x3,int n, ZARRType_c32i1b64 *v)
{
	int m[4];
    m[0] = x0%32;
    m[1] = x1%32;

...
int ZARRFlushOne_c32i1b1024(ZARR_c32i1b1024 *z, int i)
{
    if (z->written[i])
	{
	  ...
	  int compressed_len = blosc2_compress(3,1,sizeof(ZARRType_c32i1b1024),z->buffers[i],sizeof(ZARRType_c32i1b1024)*131072,z->compressedData,sizeof(ZARRType_c32i1b1024)*131072+BLOSC2_MAX_OVERHEAD);
...
int ZARRFlush_c32i1b1024(ZARR_c32i1b1024 *z)
{
	for(int i = 0; i<1024; i++)
	{
		if (z->bufferIndex[i][0] != -1)
		{
			ZARRFlushOne_c32i1b1024(z,i);
		}
	}
...
{	
	...
	for(z->index = 0; z->index < 1024; z->index++)
	{
	...
	if (z->index == 1024)
	{
		...
		ZARRFlushOne_c32i1b1024(z,oldestIndex);
		z->index = oldestIndex;
	}
...
int ZARRWrite_c32i1b1024(ZARR_c32i1b1024 *za,int x0,int x1,int x2,int x3,ZARRType_c32i1b1024 value)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
...
void ZARRWriteN_c32i1b1024(ZARR_c32i1b1024 *za,int x0,int x1,int x2,int x3,int n, ZARRType_c32i1b1024 *v)
{
	int c[4],m[4];
    c[0] = x0/32;
    m[0] = x0%32;
	...
	memcpy(&(*za->buffer)[m[0]][m[1]][m[2]][m[3]],v,n*sizeof(ZARRType_c32i1b1024));

	za->written[za->index] = 1;  
}

// Assumes that we have already written at least once to this chunk
void ZARRNoCheckWriteN_c32i1b1024(ZARR_c32i1b1024 *za,int x0,int x1,int x2,int x3,int n, ZARRType_c32i1b1024 *v)
{
	int m[4];
    m[0] = x0%32;
    m[1] = x1%32;

...
#define RAMP_DIST                       5.0f  // gravity force ramps down gradually over this distance to reduce oscillation about avgy
...
__global__ void ParticleMove(float4 *pPos, float4 *pVel, float4 *pAcc, unsigned *cellHash, unsigned *pIndex, int nParticles, int nloops, float avgy)
{
	int thdsPerBlk = blockDim.x;	// Number of threads per block
	int blkIdx = blockIdx.x;		// Index of block
	int thdIdx = threadIdx.x;		// Index of thread within a block
...
*/	
	for(int i = 0; i<nloops && zIdx < nParticles; i++)
	{
		...
		float gForce = 0.0f;
		if (pPos[zIdx].y < avgy-RAMP_DIST)
		{
		    gForce=GRAVITY_FORCE_CONSTANT;
		}
		else if (pPos[zIdx].y < avgy)
		{
		    gForce = ((avgy - pPos[zIdx].y)/RAMP_DIST) * GRAVITY_FORCE_CONSTANT;
		}
		else if (pPos[zIdx].y == 0.0f)
		{
		}
		else if (pPos[zIdx].y < avgy+RAMP_DIST)
		{
		    gForce = ((avgy - pPos[zIdx].y)/RAMP_DIST) * GRAVITY_FORCE_CONSTANT;
		}
		else
		...
		pAcc[zIdx] = make_float4(0.0f,gForce,0.0f,0.0f);
...
//	    printf("Loading...\n");
		...
		for(i = 0; i<nParticles; i++)
		{
		   ...
		   h_pPos[i].y = h_pPos[i].y - miny + Y_OFFSET;
...
//	        printf("Iteration %d\n",i);
			...
			ParticleMove<<< nblks, nthds >>>(d_pPos[activeArray],d_pVel[activeArray],d_pAcc,d_cellHash,d_pIndex,nParticles,nloops,avgy);
			thrust::sort_by_key(thrust::device_ptr<uint>(d_cellHash),
				    thrust::device_ptr<uint>(d_cellHash+nParticles),
				    thrust::device_ptr<uint>(d_pIndex));
			InitCellStart<<< nblks, nthds >>>(d_cellStart,nCellLoops);

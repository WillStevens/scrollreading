...
// Parameters related to the alignment algorithm
#define MATCH_MIN_DIST 2.0 // The minimum distance between a point in one patch and onther that will be considered before assessing it 
                           // as a match (TODO - this is probably too large)
#define NUM_POINT_SAMPLES 500 // The number of point-pair samples that will be take per-patch when looking for transforms
#define MIN_LINE_LENGTH 50 // The min length line that will be used when looking for transformations
#define MAX_LINE_DIFF 0.01 // The maximum difference in line length tolerated when looking for transformations
#define MIN_TRANSFORMS 25 // Minimum number of transforms that must be found for transform and variance to be output
...
void AlignMatches(std::map<int, std::vector<match>> &matchListMap)
{
...
int main(int argc, char * argv[])
{
	if (argc != 3)
	{
		printf("Usage: align_patches6 <patch 1> <patch 2>\n");
